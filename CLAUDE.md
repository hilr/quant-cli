...
每个**图 / 分析 / 信号 / 策略**的详细文档都单独一个文件保存，与对应代码放在同一目录（如 `plots/xxx.py` 配 `plots/xxx.md`）。README 只保留**索引信息**（一行简介 + 指向独立文档的链接），不放完整说明、参数表或结果明细。
...
生成的图（PNG 等）必须在生成后立即用 `[SEND_FILE:/absolute/path/to/file]` 标记发给用户先看，不要只打印路径就结束。
