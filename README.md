...
用 ZigZag（默认 15% 反转阈值）在沪深300 day high/low 上标记阶段高/低点，提取所有 L→H 上升段（做多机会），并在**日历年边界切分**——每段收益归到所在年份，算每年最大单段做多收益率。

- **跨年切分约定**：年尾最后一个交易日强制卖出、年头第一个交易日重新买入，相邻年份不共享边界，跨年跳空不计入任何一年。
- **未完结持仓**：最后一个枢轴若为 L（至今未反转），按 L→最新收盘价作为「持仓中」波段纳入：历史年份段用实际价（已确认），当前年份段用最新收盘价（mark-to-market）。
- 收益用实际收盘价计算（枢轴极值 high/low 仅用于定位买卖点）。
...
**分析结果（15% 阈值，沪深300，2002-2026，25 年）：**
- 每年最大做多收益均值 **32.82%**，中位 **21.68%**
- 最高 **2006 年 116.80%**（2005-2007 大牛市的涨幅主体落在 2006），最低 **2018 年 7.19%**
- 跨年大波段被正确分摊：2005-2007（#5）拆为 2005 +10.07% / 2006 +116.80%；2014-2015（#20）拆为 2014 +63.69% / 2015 +46.02%
- 2025 年已确认 **+28.99%**（持仓中波段 #30 的 2025 段），2026 至今 +7.25%
- 完整年度明细（含段起止日、价格、归属交易）见 [`plots/zigzag_annual_return.md`](plots/zigzag_annual_return.md)
