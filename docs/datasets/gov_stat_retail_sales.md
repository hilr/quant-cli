...
| 逻辑 | 「指标×月份」宽表转长表，月份按列名 `YYYY年M月` 解析（列序乱序/倒序均可）；**1-2 月合并缺口的当期值用 2 月累计值的一半补全**（合计平分：1 月 = 2 月 = 2 月累计 / 2，与官方累计值自洽） |
...
- **1-2 月合并公布（2012 起）**：2012 年起 1-2 月合并发布，当期值 1、2 月原为空，但 2 月累计值（= 1+2 月合计）有值；**输出时已用 2 月累计值的一半补全 1、2 月当期值**（合计平分）。2000-2011 年单独公布。
