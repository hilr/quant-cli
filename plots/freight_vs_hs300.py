"""货运量（铁路 + 水运）12 月滚动合计 / 同比 / 环比 vs 沪深300。

数据源 gov_stat/freight.csv（长表）。只取铁路与水运——两者均为吨位票务制
运输，计量准确。公路货运量 2014 交通运输业经济统计专项调查有小幅调整
（-5.8%），不纳入。
...
INDICATORS: dict[str, tuple[str, str]] = {
    "铁路货运量_当期值(万吨)": ("铁路", "#1f77b4"),
    "水运货运量_当期值(万吨)": ("水运", "#2ca02c"),
}
...
def plot(d: pl.DataFrame, hs300: pl.DataFrame, output_png: Path) -> None:
    ...
    def _draw(ax, col, ylabel, fmt_zero: bool):
        for ind, (lab, color) in INDICATORS.items():
            ...
            ax.plot(sub["date"], sub[col], color=color, lw=1.8,
                    label=f"{lab}货运量 {ylabel.split('（')[0]}")
        if fmt_zero:
            ax.axhline(0, color="#444", lw=0.7, ls="--", alpha=0.6)
        ax.set_ylabel(ylabel)
        ax.grid(True, alpha=0.3)
        axr = ax.twinx()
        axr.plot(hs300["date"], hs300["hs300"], color="#d62728", lw=0.9, alpha=0.5,
                 label="沪深300 月末收盘（右轴）")
    ...
    ax_lv.set_title("货运量（铁路 + 水运）12 月滚动合计 vs 沪深300")
    _draw(ax_yoy, "yoy", "同比 %", True)
    ax_yoy.set_title("货运量 12 月滚动合计 · 同比")
    _draw(ax_mom, "mom", "环比 %", True)
    ax_mom.set_title("货运量 12 月滚动合计 · 环比")
