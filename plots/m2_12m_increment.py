"""M1/M2 12 个月滚动新增额（流量），叠加沪深300月末收盘（右轴）。

12 个月新增额 = [t] − [t−12]，即过去一年净增的货币存量（万亿元）。
M2 是广义货币（含定期存款等），M1 是狭义货币（现金 + 活期存款），M1 增速
更能反映企业活化和短期流动性。右轴叠加沪深300，对照货币宽松与 A 股走势。

**M1 用 m1_new 列**（新口径 M1 可比序列，详见 pbc_money_supply.md）：
2025-01 央行扩 M1 口径（纳入个人活期存款、非银支付备付金），原始 m1 列在
该月跳涨 ~45 万亿。m1_new 通过 2024 央行官方回填 + 其余年份「旧 m1 + 住户
活期存款」近似，拼出连续新口径序列，避免 12 月增量出现假尖峰。
2011-2014 信贷表未拆住户活期，m1_new 留空，图上对应段为断线。

数据源：
- pbc/money_supply.csv 的 m1_new/m2 列（月末余额，亿元）
...
def load_money_12m(money_file: Path) -> pl.DataFrame:
    """读 M1（m1_new 新口径）/M2 月末余额，求各自 12 月滚动增量，换算万亿元。"""
    return (pl.read_csv(money_file)
              ...
              .with_columns([
                  ...
                  ((pl.col("m1_new") - pl.col("m1_new").shift(12)) / 10000).alias("m1_12m_增量_万亿"),
              ]))
...
def plot(d: pl.DataFrame, hs300: pl.DataFrame, output_png: Path) -> None:
    ...
    ax.set_title("M1（新口径）/ M2 货币 12 个月新增额 vs 沪深300")
    ax.set_ylabel("万亿元")
...
def main() -> None:
    ...
    d = load_money_12m(args.money_file)
    hs300 = load_hs300(args.index_file)
    miss = (d.filter(pl.col("m1_12m_增量_万亿").is_null() | pl.col("m2_12m_增量_万亿").is_null())
              ["date"].to_list())
    if miss:
        print(f"警告：以下月份缺失（shift 头 12 月 + 2011-2014 信贷表未拆住户活期的 m1_new null）：{miss[:6]}{' ...' if len(miss) > 6 else ''}")
